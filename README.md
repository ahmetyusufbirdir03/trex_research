...
| No                                                 | Açıklama                                                                                | Örnek                                                                                                                                                                                              |
| -------------------------------------------------- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A01 – Broken Access Control**                    | Kullanıcıların erişim yetkisi olmayan işlemleri yapabilmesi veya verilere ulaşabilmesi. | ❌ **Kötü:**<br>`csharp public IActionResult Delete(int id) => _repo.Delete(id);`<br>✅ **Doğru:**<br>`csharp [Authorize(Roles="Admin")] public IActionResult Delete(int id) { ... }`                |
| **A02 – Cryptographic Failures**                   | Hassas verilerin (şifre, kart, token) açık veya zayıf şekilde saklanması.               | ❌ **Kötü:**<br>`csharp user.Password = password;`<br>✅ **Doğru:**<br>`csharp using var hmac = new HMACSHA512(); user.PasswordHash = hmac.ComputeHash(Encoding.UTF8.GetBytes(password));`           |
| **A03 – Injection (SQL Injection)**                | Kullanıcı girdisinin doğrudan SQL sorgusuna eklenmesi.                                  | ❌ **Kötü:**<br>`csharp var q = "SELECT * FROM Users WHERE Name='" + name + "'";`<br>✅ **Doğru:**<br>`csharp var user = db.Users.FirstOrDefault(u => u.Name == name);`                              |
| **A04 – Insecure Design**                          | Güvenlik tasarım aşamasında düşünülmemişse, sistem açık hale gelir.                     | ❌ **Kötü:** Aynı API anahtarı tüm kullanıcılarca paylaşılıyor.<br>✅ **Doğru:** Her kullanıcıya özel token (`Guid.NewGuid()`) ve süre sınırı kullan.                                                |
| **A05 – Security Misconfiguration**                | Framework veya sunucu yanlış yapılandırılmıştır (örneğin debug açık kalmış).            | ❌ **Kötü:**<br>`"ConnectionStrings": { "Default": "Server=.;User=sa;Pwd=1234" }`<br>✅ **Doğru:** Gizli veriler `Secret Manager` veya `Azure Key Vault` içinde saklanmalı.                          |
| **A06 – Vulnerable and Outdated Components**       | Eski veya güvenlik açığı olan kütüphaneler kullanmak.                                   | ❌ **Kötü:** Güncel olmayan paket sürümleri.<br>✅ **Doğru:** `dotnet list package --outdated` ile düzenli kontrol et.                                                                               |
| **A07 – Identification & Authentication Failures** | Kimlik doğrulama veya oturum yönetimi zayıfsa kullanıcı hesabı ele geçirilebilir.       | ❌ **Kötü:**<br>`csharp if (username=="admin" && password=="1234")`<br>✅ **Doğru:**<br>JWT token + `[Authorize]` kullan.<br>`csharp new JwtSecurityToken(expires: DateTime.UtcNow.AddMinutes(15));` |
| **A08 – Software & Data Integrity Failures**       | Kod veya verinin bütünlüğü doğrulanmıyorsa zararlı kod çalıştırılabilir.                | ❌ **Kötü:** İmzasız güncelleme dosyası yüklemek.<br>✅ **Doğru:** Paket imzalarını kontrol et (`dotnet nuget verify`) ve hash doğrulaması yap.                                                      |
